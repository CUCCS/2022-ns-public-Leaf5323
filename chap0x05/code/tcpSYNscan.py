...
def createSYNpacket(destinationIP,destinationPort):
    packet=sli.IP(dst=destinationIP)/sli.TCP(dport=destinationPort,flags="S")
    return packet

def createRSTpacket(destinationIP,destinationPort):
    packet=sli.IP(dst=destinationIP)/sli.TCP(dport=destinationPort,flags="R")
    return packet
...
if __name__=='__main__':
    if len(argv)<2:
        ...
        tcpSYNscan(targetIP=argv[0], targetPort=argv[1])